...
AS
  ...
  --      SET     Net_Percent = Omola_Percent
        ...
        UPDATE  #Total
        SET     TargetPercent = ( ( TotalSales ) / ( Targetvalue ) ) * 100



        UPDATE  [#Total]
        SET     net_Omola = ( SELECT  TOP 1  OmolaValue * ( Target_Omola.DuePercent
                                                       / 100 )
                              FROM      Target_Omola
                              WHERE     [#Total].MandopID = Target_Omola.MandopID
                                        AND Target_Omola.ActPersent <= [#Total].TargetPercent
                              ORDER BY  DuePercent DESC
                            )	 
