...
CREATE   PROCEDURE [dbo].[Documentary_of_Guarantee_Balance]
AS
    BEGIN
        ...
        INSERT  INTO #Result
                ...
                SELECT  Collateral_ID ,
                        ...
                        #Detailstbl.Details ,
                        0
                FROM    dbo.Documentary_of_Guarantee
                        INNER JOIN dbo.Documentary ON Documentary.ID = Documentary_of_Guarantee.Collateral_ID
                        CROSS JOIN #Detailstbl
                        INNER JOIN dbo.Bank_Code ON Documentary_of_Guarantee.Bank_ID = Bank_Code.id_Bank
        ...
        SET     Amount = ( SELECT    ISNULL(SUM(Collateral_Value),0)
                           FROM     dbo.Documentary_of_Guarantee
                           WHERE    Documentary_of_Guarantee.Bank_ID = #Result.BanK_ID
                                    AND Documentary_of_Guarantee.Collateral_ID = #Result.Type_of_Guarantee_ID
                         )
        ...
        SET     Amount = ( SELECT   ISNULL(SUM(Collateral_Value),0)
                           ...
                           WHERE    Documentary_of_Guarantee_Used.Bank_ID = #Result.BanK_ID
                                    AND Documentary_of_Guarantee_Used.Collateral_ID = #Result.Type_of_Guarantee_ID
                         )
        WHERE   Details_ID = 2;
